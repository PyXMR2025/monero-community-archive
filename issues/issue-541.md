---
title: Broadcast new blocks before removing txs from the pool
source_url: https://github.com/Cuprate/cuprate/issues/541
author: Boog900
assignees: []
labels:
- C-proposal
created_at: '2025-08-30T16:29:35+00:00'
updated_at: '2026-05-05T13:16:20+00:00'
type: issue
status: closed
closed_at: '2026-05-05T13:16:20+00:00'
---

# Original Description

## What
Broadcast blocks to the network before we tell the tx-pool about the new block.

## Why
To speed up block propagation 

## Where
cuprated




# Discussion History
# Action History
- Created by: Boog900 | 2025-08-30T16:29:35+00:00
- Closed at: 2026-05-05T13:16:20+00:00
