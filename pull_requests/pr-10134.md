---
title: 'crypto: break out fe_pow22523 into separate function'
source_url: https://github.com/monero-project/monero/pull/10134
author: j-berman
assignees: []
labels:
- pending review
- cryptography
created_at: '2025-09-30T19:58:56+00:00'
updated_at: '2026-07-09T18:41:09+00:00'
type: pull_request
status: open
closed_at: null
merged_at: null
---

# Original Description
Used in [sqrt](https://github.com/seraphis-migration/monero/blob/da7bbef285278a1368052730215cb405b35aadd8/src/fcmp_pp/fcmp_pp_crypto.cpp#L75) implemented in FCMP++, which is used in this [`torsion_check_vartime`](https://github.com/seraphis-migration/monero/blob/da7bbef285278a1368052730215cb405b35aadd8/src/fcmp_pp/fcmp_pp_crypto.cpp#L259-L267).

`torsion_check_vartime` is part of an optimization* that wallets use to check outputs and commitments for torsion, before clearing torsion if they do indeed have torsion (most are expected to not have torsion, hence this optimization). Outputs in the tree cannot have torsion. Wallets build the tree locally, and therefore need to make sure torsioned outputs do not enter the tree.

*`torsion_check_vartime` is currently unproven. That is why the integration presently only uses it wallet-side in order to speed up wallet sync, and not at consensus. Wallet-side worst case is a DoS. Daemon-side worst case is inflation. Theoretically we could hold off on using it wallet-side until after the fork, at which point we ban torsioned outputs at consensus, and can therefore be certain the function works correctly for all pre-fork potentially torsioned outputs.

# Discussion History
## jeffro256 | 2026-07-09T18:41:09+00:00
In case it's not obvious, I recommend to add a docstring that this function is calculating `out` = $z^{2^{252}-3}$, and what the preconditions of `z` are (same as `fe_mul()` and `fe_sq()`). 

# Action History
- Created by: j-berman | 2025-09-30T19:58:56+00:00
