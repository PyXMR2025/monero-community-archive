...
updated_at: '2026-05-14T16:04:36+00:00'
type: pull_request
status: open
closed_at: null
merged_at: null
...
- `lib.rs` (new) - `Node` struct, `NodeContext` for shared state, `Node::launch()`
- `main.rs` - slimmed to a CLI wrapper over `Node::launch()`
- `commands.rs` - `CommandHandle` actor with mpsc/oneshot `send_command()` API
...
`consensus/fast-sync`:
- `FAST_SYNC_HASHES` static + `set_fast_sync_hashes` removed; `fast_sync_stop_height` and `validate_entries` take hashes as a parameter from NodeContext
...
Internal state previously held in mutable statics lives in NodeContext - the node is now safe to run multiple times in a single process.

2. The new `BlockchainInterface` composes three handles into one type for ergonomics:
```rust
...
3. The Tor interface arrives via oneshot after sync, similar to the `dandelion.rs` pattern.
