---
title: 'Investigating repeated LMDB errors: could a MITM / malicious peers be pushing
  a covert fork?'
source_url: https://github.com/monero-project/monero/issues/10140
author: danindiana
assignees: []
labels:
- question
created_at: '2025-10-03T00:37:28+00:00'
updated_at: '2025-10-03T11:27:40+00:00'
type: issue
status: open
closed_at: null
---
...
# Discussion History
## danindiana | 2025-10-03T00:42:24+00:00
Status update: posted sanitized logs and diagnostics (bitmonero.log snippet, monerod status, peerlist, SMART, fsck) as artifact comments.

Current state:
- Fresh sync restarted (LMDB wiped) and running with db-sync-mode=safe.
- SMART long self-test is in progress on the NVMe device.

Immediate notes:
- If corruption recurs I will capture and attach the exact failing height/hash, a short pcap of the peer(s) active at the time, and the relevant rpc responses.

I will also add a follow-up comment with exact RPC and pcap commands to run at failure time (so maintainers or other node operators can reproduce minimal forensic captures).

--
Posted by automated upload from user-provided sanitized artifacts.


## danindiana | 2025-10-03T02:14:31+00:00
**Forensic Update: Corruption recurred with identical pattern**

Issue happened again at height 2110776. Key evidence:

**Identical transaction hash repeatedly failing:**
- 
- Pattern: 
- Same error repeatedly occurs trying to process this exact same transaction

**Peer behavior:**
-  was blocked during the failure window
- Multiple P2P threads (P2P1, P2P2, P2P5, P2P6, P2P7, P2P8, P2P9) all hit the same transaction hash error

**Timeline pattern:**
- 01:37:49 to around 02:01:49: continuous MDB_PAGE_NOTFOUND for the same tx hash
- Daemon status shows sync was at 
- Local RPC confirms our height is exactly 2110775 (one behind the failing block)

**Key observation:**
This is the **exact same transaction hash** that failed before. This strongly suggests either:
1. Corrupted/malicious data for this specific transaction is being distributed by multiple peers
2. Our local LMDB corruption is persistent and affects this specific transaction index

**Next forensic steps:**
- Need to compare the transaction data for  from multiple authoritative sources
- Capture the exact block containing this transaction from trusted vs suspect peers
- Block height 2110776 appears to be the failing point

Currently running with db-sync-mode=safe but corruption still occurred.


## nahuhh | 2025-10-03T02:39:51+00:00
What is the hash

## danindiana | 2025-10-03T05:22:31+00:00
@nahuhh 

exact same transaction hash (552c9a41391d4be1273f05d9ca8b135acf112db991ae3d27677f6040fc5fe324) is repeatedly causing MDB_PAGE_NOTFOUND errors
Your daemon is stuck at height 2110775, failing to process block 2110776
Peer 209.222.252.225 was blocked during the failure window
Even with db-sync-mode=safe, the corruption still occurred

## nahuhh | 2025-10-03T11:27:22+00:00
 
> exact same transaction hash (552c9a41391d4be1273f05d9ca8b135acf112db991ae3d27677f6040fc5fe324) is repeatedly causing MDB_PAGE_NOTFOUND errors
> Your daemon is stuck at height 2110775, failing to process block 2110776

that tx is in block 2110790

> Peer 209.222.252.225 was blocked during the failure window

this node is a known linkinglion spy node
...
# Action History
- Created by: danindiana | 2025-10-03T00:37:28+00:00
